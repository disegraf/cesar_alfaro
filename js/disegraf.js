(function($) {
    ...
    function de_percentage_counter() {
        jQuery('.percent').each(function() {
            var $this = jQuery(this);
            var imagePos = $this.offset().top;
            var topOfWindow = jQuery(window).scrollTop();

            if (imagePos < topOfWindow + jQuery(window).height() && !$this.hasClass('counted')) {
                $this.addClass('counted');

                $this.countTo({
                    from: 0,
                    to: $this.data('percent'),
                    speed: 2000,
                    refreshInterval: 50,
    ...
    function de_percentage_counter2() {
        jQuery('.custom-bar-fire-container').each(function() {
            var $container = jQuery(this);
            var $progressBar = $container.find('.progress-bar');
            var $span = $container.find('span').last();
            var percent = parseInt($span.text(), 10);

            var imagePos = $container.offset().top;
            var topOfWindow = jQuery(window).scrollTop();

            if (imagePos < topOfWindow + jQuery(window).height() && !$container.hasClass('counted')) {
                $container.addClass('counted');

                jQuery({ value: 0 }).animate({ value: percent }, {
                    duration: 2000,
                    step: function(now) {
     ...
     $(function(){
        ...
        jQuery.each(jQuery('textarea[data-autoresize]'), function() {
         ...
         jQuery(window).on("scroll", function() {
             header_sticky();
             de_counter();
             de_percentage_counter();
             de_percentage_counter2();
             de_progress();
             init();
             backToTop();
             moveItItemNow();
